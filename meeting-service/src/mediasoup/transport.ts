...
function attachTransportEvents(transport: WebRtcTransport) {
  transport.on("icestatechange", (iceState) => {
    logger.info(
      `[Transport] ICE state: ${iceState} for transport ${transport.id}`
    );
  });

  transport.on("dtlsstatechange", (dtlsState) => {
    logger.info(
      `[Transport] DTLS state: ${dtlsState} for transport ${transport.id}`
    );

    if (dtlsState === "failed" || dtlsState === "closed") {
      logger.error(
        `[Transport] Transport ${transport.id} DTLS state ${dtlsState}`
      );
...
export async function createWebRtcTransport(
  ...
  const candidateSummary = transport.iceCandidates
    ?.map(
      (c) =>
        `${c.protocol}:${c.ip || (c as any).address}:${c.port}`
    )
    .join(", ");

  logger.info(
    `[Transport] WebRTC transport created: id=${transport.id} ` +
